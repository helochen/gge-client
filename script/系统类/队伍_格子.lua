...
function 系统类_队伍格子:置人物(人物)
	...
	if 人物 ~= nil then
		...
		if  人物.名称 ~= nil and 人物.名称 ~= 0 then
            ...
            elseif  人物.坐骑 ~= nil and (#人物.坐骑 ~= nil or #人物.坐骑 ~= 0) and 人物.坐骑.模型 ~= nil and (人物.锦衣[1]==nil or 引擎.取站立坐骑(人物.坐骑.模型) ~= nil) then
            	local 资源组 = {}
            	local 站立坐骑 = 0
            	if 引擎.新增坐骑(人物.模型,人物.坐骑.模型,"奔跑") ~= "" then
					资源组 = {}
					资源组.人物资源 = "xzsc.wdf" or "wpal/121.wpal"
					资源组.人物站立 = 引擎.新增坐骑(人物.模型,人物.坐骑.模型,"站立")
					资源组.坐骑资源 = "xzsc.wdf" or "wpal/121.wpal"
					资源组.坐骑站立 = 引擎.新增坐骑(人物.模型,人物.坐骑.模型,"站立")
				elseif 引擎.取站立坐骑(人物.坐骑.模型) ~= nil then
					站立坐骑 = 1
					资源组.坐骑资源 = "xzsc.wdf" or "wpal/121.wpal"
					资源组.坐骑站立 = 引擎.取站立坐骑(人物.坐骑.模型)[2]
				else
					资源组 = zqj(人物.模型,人物.坐骑.模型,人物.坐骑.饰品 or "空")
				end
				if 站立坐骑 == 0 then
				...
				if 人物.染色方案~=nil and 人物.坐骑.模型 ~= "独角兽" and 人物.坐骑.模型 ~= "独角兽1" and 人物.坐骑.模型 ~= "飞天龙马"  and 人物.坐骑.模型 ~= "霜雪龙宝" and 人物.坐骑.模型 ~= "冰晶雪魄" and 人物.坐骑.模型 ~= "冰晶魅灵" and 人物.坐骑.模型 ~= "青梅竹马"  and 人物.坐骑.模型 ~= "深海狂鲨"  and 人物.坐骑.模型 ~= "独角马" and 人物.坐骑.模型 ~= "海豚恋人" and 人物.坐骑.模型 ~="冰晶魅灵" and 人物.坐骑.模型 ~="炫影天马" and 人物.坐骑.模型 ~="玲珑雪狐" and 人物.坐骑.模型 ~="玲珑雪狐女" and 人物.坐骑.模型 ~="烈焰雄狮" and 人物.坐骑.模型 ~="九尾神狐1" then
				    if (人物.锦衣[1] ~= nil and 人物.锦衣[1].名称 ~= nil) or 人物.变身数据 ~= nil  then
				    else
				   	 	self.模型:置染色(人物.染色方案,人物.染色组[1],人物.染色组[2],人物.染色组[3])
				   	end
				...
				self.模型:置方向(4)
				self.坐骑:置方向(4)
				self.b.x,self.b.y = 引擎.补差(人物.坐骑.模型,人物.模型)
...
function 系统类_队伍格子:显示(dt,x,y,条件,数据,x1,y1)
	...
	if self.模型 ~= nil and 人物 ~= nil then
		...
		if self.模型身体 ~= nil then
			self.模型身体:显示(self.x + 54+self.b.x,self.y + 117+self.b.y)
		end
		if self.足迹 ~= nil then
			self.足迹:显示(self.x + 54,self.y + 117)
		end
		self.模型:显示(self.x + 54+self.b.x,self.y + 117+self.b.y)
		if self.武器 ~= nil then
			self.武器:显示(self.x + 54+self.b.x,self.y + 117+self.b.y)
		end
	end
end
