...
-- @Last Modified time: 2020-08-22 01:24:48
--======================================================================--
local 系统类_鼠标 = class()
...
function 系统类_鼠标:初始化(根)
	local 资源 = 根.资源
	指针["普通"] = 资源:载入('wzife.wd5',"网易WDF动画",0x81DD40DC)
	...
	--指针["添加好友"] = 资源:载入('wzife.wdf',"网易WDF动画",0x31A416A7)
	指针["添加好友"] = 资源:载入('other.wdf',"网易WDF动画",0x87540E93)
	当前 = "普通"
	记忆当前 = 当前
	tp = 根
end
...
function 系统类_鼠标.置鼠标(v)
	if 当前 == v or 当前 == "添加好友" or 当前=="组队" or 当前=="给予" or 当前=="交易"  or 当前=="平时攻击" then
		return false
	end
	if v == "给予" or v == "添加好友" or v == "交易" or v == "组队" or v == "普通" or v == "输入" or v == "事件" or v == "平时攻击"  or (tp.场景 ~= nil and tp.战斗中 and v == "攻击") then
		tp.隐藏UI = false
	else
		tp.隐藏UI = true
	end
...
function 系统类_鼠标:更新(dt,x,y)
	if 引擎.鼠标弹起(右键) then
		if 当前=="组队" or 当前=="添加好友" or 当前=="给予" or 当前=="交易"  or 当前=="平时攻击" then
			当前 = "普通"
		end
	end
	指针[当前]:更新(dt)
