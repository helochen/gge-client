...
function 藏宝阁银子出售:显示(dt,x,y)
	...
	elseif self.存入:事件判断() then
		...
		elseif 	tonumber(self.出售银两:取文本()) < 199999999 then
			...
			return 0
		elseif 	tonumber(self.出售银两:取文本()) >= 5000000000 then
			tp.提示:写入("#y/上架金额不能超过50亿银两")
			return 0
		elseif 	tonumber(self.出售价格:取文本()) >= 1000 then
			tp.提示:写入("#y/上架金额不能超过一千点卡")
			return 0
		end
        发送数据(108,{编号=self.出售银两:取文本()+0,价格=self.出售价格:取文本()+0})
        self.可视 = false
	end
