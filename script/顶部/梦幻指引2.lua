...
function 场景类_梦幻指引:打开(数据)
	if self.可视 then
		...
		self.日常任务=tp.怪物信息
		self.限时活动=tp.活动信息
		self.翻页数据 = math.floor(#self.日常任务/6)
		self.按键数组={}
		for i=1,6 do
			self.按键数组[#self.按键数组+1]=按钮.创建(pwd("11"),0,0,4,true,true,"查看攻略")
		end

		self.zts =require("gge文字类").创建(nil,20,false,false,true)
	end
end
function 场景类_梦幻指引:加载数据()
	...
	self.资源组 = {
		...
		[5] = 按钮.创建(pwd("1"),0,0,4,true,true,"				地图活动"),
		[6] = 按钮.创建(pwd("1"),0,0,4,true,true,"				固定活动"),
...
function 场景类_梦幻指引:显示(dt,x,y)
	...
	zts:显示(self.x+365,self.y+390,"当前活跃："..self.数据.活跃数据.活跃度)
	zts:显示(self.x+365,self.y+405,"累积活跃："..self.数据.累积活跃.当前积分)
	zts:显示(self.x+308,self.y+430,"500活跃度")
	zts:显示(self.x+475,self.y+430,"活动日历")
	zts:显示(self.x+550,self.y+430,"排 行")
	zts:显示(self.x+601,self.y+430,"兑 换")
	zts2:显示(self.x+508,self.y+401,os.date("%d", os.time()).."日")
	self.zts:显示(self.x+308,self.y+0, self.注释)
	local xx=0
	local yy=0
	if self.状态 == 1 then--日常任务
		local bbsa = #self.日常任务
		...
		for i=1,6 do
			...
			if self.日常任务[i]~=nil then
				local x = 引擎.取物品(self.日常任务[i].图标)
				self.资源组[20]=tp.资源:载入(x[11],"网易WDF动画",x[12])
			end

			if self.日常任务[i+self.加入*6]~=nil then
				self.按键数组[i]:更新(x,y)
				...
				self.按键数组[i]:显示(self.x+88+158+xx*280,self.y+92+10+yy*85)
				zts:置颜色(黑色)
				zts:显示(self.x+88+58+xx*280,self.y+92+15+yy*85,self.日常任务[i+self.加入*6].名称)
				if self.日常任务[i+self.加入*6].活跃数据.每次~=nil then
					zts:显示(self.x+88+58+xx*280,self.y+92+45+yy*85,"活跃度：每次"..self.日常任务[i+self.加入*6].活跃数据.每次.."点X"..self.日常任务[i+self.加入*6].活跃数据.限制.."次")
				end
			end
			if self.按键数组[i]:事件判断() then
			    tp.窗口.玩法介绍:打开(self.日常任务[i+self.加入*6],"日常活动")
			end
			xx=xx+1
		end
	...
	elseif self.状态 == 2 then--玩法介绍
		...
		for i=1,6 do
			...
			if self.限时活动[i+self.加入*6]~=nil then
				local x = 引擎.取物品(self.限时活动[i+self.加入*6].图标)
				self.资源组[20]=tp.资源:载入(x[11],"网易WDF动画",x[12])
			end
			...
			if self.限时活动[i+self.加入*6]~=nil then
				...
				self.按键数组[i]:显示(self.x+88+158+xx*280,self.y+92+10+yy*85)
				zts:置颜色(黑色)
				zts:显示(self.x+88+58+xx*280,self.y+92+15+yy*85,self.限时活动[i+self.加入*6].名称)
				if self.限时活动[i+self.加入*6].活跃数据.每次~=nil then
					zts:显示(self.x+88+58+xx*280,self.y+92+45+yy*85,"活跃度：每次"..self.限时活动[i+self.加入*6].活跃数据.每次.."点X"..self.限时活动[i+self.加入*6].活跃数据.限制.."次")
				end
			end
			if self.按键数组[i]:事件判断() then
			   tp.窗口.玩法介绍:打开(self.限时活动[i+self.加入*6],"限时活动")
			end
			xx=xx+1
		end
		self.资源组[17]:显示(self.x+620,self.y+80-16)
