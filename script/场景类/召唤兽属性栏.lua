    ...
    if self.可视 then
        ...
        for i = 1, #tp.队伍[1].宝宝列表 do
            if tp.队伍[1].宝宝列表[i] ~= nil and tp.队伍[1].宝宝列表[i].参战信息 ~= nil then
                ...
                self.选中 = i
                self.名称输入框:置文本(tp.队伍[1].宝宝列表[self.选中].name)
                self.名称输入框:置可视(true, true)
                if tp.窗口.召唤兽资质栏.可视 then
                    tp.窗口.召唤兽资质栏:PB打开(tp.队伍[1].宝宝列表[self.选中])
                end
                self:PB置形象()
                self.拽拖计次 = 0
                break
...
function 场景类_召唤兽属性栏:加载数据()
    ...
    self.资源组 = {
        ...
        [17] = 按钮.创建(自适应.创建(12, 4, 43, 22, 1, 3), 0, 0, 4, true, true, '繁殖'),
        [18] = 按钮.创建(自适应.创建(12, 4, 43, 22, 1, 3), 0, 0, 4, true, true, '放生'),
        [19] = 按钮.创建(自适应.创建(12, 4, 43, 22, 1, 3), 0, 0, 4, true, true, '鉴定'),
        [20] = 按钮.创建(自适应.创建(12, 4, 43, 22, 1, 3), 0, 0, 4, true, true, '认证'),
        [21] = 按钮.创建(自适应.创建(12, 4, 72, 22, 1, 3), 0, 0, 4, true, true, '确认加点'),
        [22] = 按钮.创建(自适应.创建(12, 4, 72, 22, 1, 3), 0, 0, 4, true, true, '查看资质'),
        [23] = 资源:载入('wzife.wdf', '网易WDF动画', 0x363AAF1B),
        [26] = 资源:载入('wzife.wd4', '网易WDF动画', 0x3906F9F1),
        --经验
        [27] = 按钮.创建(自适应.创建(20, 4, 18, 19, 4, 3), 0, 0, 4, true, true),
        --上滚动
        [28] = 按钮.创建(自适应.创建(21, 4, 18, 19, 4, 3), 0, 0, 4, true, true),
        --下滚动
        [29] = 自适应.创建(34, 1, 173, 182, 3, 9),
        --召唤兽选择框
        [30] = 资源:载入(tp.造型背景, '网易WDF动画', 0xA19838E8),
        --宠物显示框
        ...
        [37] = 按钮.创建(自适应.创建(12, 4, 43, 22, 1, 3), 0, 0, 4, true, true, '扩展')
    }
    self.资源组[22]:置偏移(0, 3)
    for i = 2, 28 do
    ...
    local 格子 = require('script/显示类/物品_格子2')
    self.饰品 = {}
    for i = 1, 1 do
        self.饰品[i] = 格子(0, 0, i, '道具行囊_灵饰', 资源:载入('wzife.wd2', '网易WDF动画', 0xA4F1E391))
    end
    self.追加法术 = 资源:载入('wzife.wdf', '网易WDF动画', 0xef45cdc6)
end

function 场景类_召唤兽属性栏:PB置形象()
    self.资源组[24] = nil
    self.资源组[25] = nil
    self.资源组[40] = nil

    if tp.队伍[1].宝宝列表[self.选中] ~= nil then
        local bbDress = tp.队伍[1].宝宝列表[self.选中].dress
        if bbDress ~= nil then
            if gbk.len(bbDress.name) > 0 then
                n = 引擎.取模型(bbDress.name .. '_饰品')
                self.资源组[25] = tp.资源:载入(n[3], '网易WDF动画', n[1])
                self.资源组[25]:置方向(0)
            end
            local wp = tp._物品
            local qq = wp()
            qq:置对象(tp.队伍[1].宝宝列表[self.选中].model .. '饰品')
            self.饰品[1]:置物品(qq)
            self.饰品[1].物品.总类 = 57
            self.饰品[1].物品.玄天灵力 = bbDress.mana or 0
            if bbDress.color ~= nil and #bbDress.color > 0 then
                self.资源组[25]:置染色(bbDress.color[4], bbDress.color[1], bbDress.color[2], bbDress.color[3])
                self.资源组[25]:置方向(0)
            end
        end

        if tp.队伍[1].宝宝列表[self.选中].model == '贪狼' then
            self.资源组[24] = pwd('53')
            self.资源组[24]:置方向(0)
        else
            local n = ani(tp.队伍[1].宝宝列表[self.选中].model)
            self.资源组[24] = tp.资源:载入(n[10], '网易WDF动画', n[6])
            self.资源组[24]:置方向(0)
        end
        if tp.队伍[1].宝宝列表[self.选中].color ~= nil and #tp.队伍[1].宝宝列表[self.选中].color > 0 then
            self.资源组[24]:置染色(
                tp.队伍[1].宝宝列表[self.选中].color[4],
                tp.队伍[1].宝宝列表[self.选中].color[3],
                tp.队伍[1].宝宝列表[self.选中].color[2],
                tp.队伍[1].宝宝列表[self.选中].color[1]
            )
            self.资源组[24]:置方向(0)
        end
    end
...
function 场景类_召唤兽属性栏:置形象()
    ...
    if tp.队伍[1].宝宝列表[self.选中] ~= nil then
        if tp.队伍[1].宝宝列表[self.选中].饰品 ~= nil then
            n = 引擎.取模型(tp.队伍[1].宝宝列表[self.选中].模型 .. '_饰品')
            self.资源组[25] = tp.资源:载入(n[3], '网易WDF动画', n[1])
            self.资源组[25]:置方向(0)
            ...
            if tp.队伍[1].宝宝列表[self.选中].饰品.染色方案 ~= nil then
                self.资源组[25]:置染色(
                    tp.队伍[1].宝宝列表[self.选中].饰品.染色方案,
                    tp.队伍[1].宝宝列表[self.选中].饰品.染色组[1],
                    tp.队伍[1].宝宝列表[self.选中].饰品.染色组[2],
                    tp.队伍[1].宝宝列表[self.选中].饰品.染色组[3]
                )
                self.资源组[25]:置方向(0)
            end
        end
        ...
        if tp.队伍[1].宝宝列表[self.选中].模型 == '贪狼' then
            ...
            if tp.队伍[1].宝宝列表[self.选中].染色方案 ~= nil and tp.队伍[1].宝宝列表[self.选中].染色组 ~= nil then
                self.资源组[24]:置染色(
                    tp.队伍[1].宝宝列表[self.选中].染色方案,
                    tp.队伍[1].宝宝列表[self.选中].染色组[1],
                    tp.队伍[1].宝宝列表[self.选中].染色组[2],
                    tp.队伍[1].宝宝列表[self.选中].染色组[3]
                )
                self.资源组[24]:置方向(0)
            end
        else
...
function 场景类_召唤兽属性栏:PB显示(dt, x, y)
    ...
    if self.选中 ~= 0 then
        ...
        if bb.助战参战 == nil then
            if bb ~= nil and bb.join ~= nil and bb.join == false then
                self.资源组[3]:置文字('休息')
            else
                self.资源组[3]:置文字('参战')
            end
    ...
    self.资源组[34]:更新(x, y)
    for i = 5, 9 do
        self.资源组[i]:更新(x, y, bb ~= nil and bb.potential > 0, 1)
        if self.资源组[i]:事件判断() then
            bb.potential = bb.potential - 1
    ...
    self.资源组[15]:更新(x, y, bb ~= nil and bb.potential > 0)
    ...
    zts2:置字间距(0)
    self.资源组[37]:显示(self.x + 153, self.y + 218)
    ...
    zts:置颜色(4294967295)
    zts:显示(self.x + 10, self.y + 222, '召唤兽携带数量：' .. bbsa .. '/12')
    zts:显示(self.x + 21, self.y + 427, '经验')
    if bb == nil then
        zts:显示(self.x + 200, self.y + 194, '参战等级：--')
    else
        zts:显示(self.x + 200, self.y + 194, '参战等级：' .. bb.fightLv)
    end
    for i = 0, 5 do
    ...
    self.资源组[22]:显示(self.x + 288, self.y + 418, true)

    -- TODO 饰品物品
    if bb ~= nil and bb.dress ~= nil then
        self.饰品[1]:置坐标(self.x + 200, self.y + 145)
        self.饰品[1]:显示(dt, x, y, self.鼠标)
        if self.饰品[1].物品 ~= nil and self.饰品[1].焦点 then
            tp.提示:道具行囊(x, y, self.饰品[1].物品, true)
        end
    end
    ...
    for i = 1, 4 do
        if bbs[i + self.加入] ~= nil then
            ...
            if self.头像组[i + self.加入] == nil then
                local n = tx(bbs[i + self.加入].model)
            ...
            if self.选中 ~= i + self.加入 then
                if not self.资源组[35].接触 and xz and not tp.消息栏焦点 and self.鼠标 then
                    ...
                    if mouseb(0) and self.拽拖对象 == 0 then
                        self.选中 = i + self.加入
                        self.名称输入框:置文本(bbs[self.选中].name)
                        self.名称输入框:置可视(true, true)
                        if tp.窗口.召唤兽资质栏.可视 then
            ...
            if bbs[i + self.加入].join ~= nil then
                zts:置颜色(-256)
            else
                zts:置颜色(-16777216)
            end
    ...
    if bb ~= nil then
        ...
        self.资源组[36]:显示(self.x + 200, self.y + 40)
        ...
        if tp.队伍[1].宝宝列表[self.选中].type ~= nil and gbk.len(tp.队伍[1].宝宝列表[self.选中].type) then
            local 资源 = tp.资源
            if tp.队伍[1].宝宝列表[self.选中].type == '神兽' then
                self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb118)
                self.资源组[41]:更新(x, y)
                self.资源组[41]:显示(self.x + 295, self.y + 145)
            elseif tp.队伍[1].宝宝列表[self.选中].type == '宝宝' then
                self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb116)
                self.资源组[41]:更新(x, y)
                self.资源组[41]:显示(self.x + 295, self.y + 145)
            elseif tp.队伍[1].宝宝列表[self.选中].type == '仙灵' then
                self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb117)
                self.资源组[41]:更新(x, y)
                self.资源组[41]:显示(self.x + 295, self.y + 145)
            elseif tp.队伍[1].宝宝列表[self.选中].type == '妖兽' then
                self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb115)
                self.资源组[41]:更新(x, y)
                self.资源组[41]:显示(self.x + 295, self.y + 145)
            elseif tp.队伍[1].宝宝列表[self.选中].type == '至尊' then
                self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb114)
                self.资源组[41]:更新(x, y)
                self.资源组[41]:显示(self.x + 295, self.y + 145)
            else
                --########################################################?自己修改?##########################################
                ztstt3:显示(self.x + 204, self.y + 40, tp.队伍[1].宝宝列表[self.选中].type)
            end
        end
        ...
        local evoAttr = bb.evoAttr

        if evoAttr ~= nil and #evoAttr > 0 then
            if evoAttr[1] > 0 then
                zts:置颜色(0xFFFF00FF)
                zts:显示(self.x + 272, self.y + 247, '+' .. evoAttr[1])
                zts:置颜色(-16777216)
            end
            if evoAttr[2] > 0 then
                zts:置颜色(0xFFFF00FF)
                zts:显示(self.x + 272, self.y + 271, '+' .. evoAttr[2])
                zts:置颜色(-16777216)
            end
            if evoAttr[3] > 0 then
                zts:置颜色(0xFFFF00FF)
                zts:显示(self.x + 272, self.y + 295, '+' .. evoAttr[3])
                zts:置颜色(-16777216)
            end
            if evoAttr[4] > 0 then
                zts:置颜色(0xFFFF00FF)
                zts:显示(self.x + 272, self.y + 319, '+' .. evoAttr[4])
                zts:置颜色(-16777216)
            end
            if evoAttr[5] > 0 then
                zts:置颜色(0xFFFF00FF)
                zts:显示(self.x + 272, self.y + 343, '+' .. evoAttr[5])
                zts:置颜色(-16777216)
            end
        end
        ...
        --     zts:置颜色(-16777216)
        -- end
        zts:显示(self.x + 200, self.y + 367, bb.potential)
        self.资源组[26]:置区域(0, 0, min(floor(bb.exp[1] / bb.exp[2] * 176), 176), 16)
        self.资源组[26]:显示(self.x + 60, self.y + 426)
        zts2:置描边颜色(-16240640)
        zts2:置颜色(4294967295)
        zts2:置描边颜色(-16777216)
        local ts = format('%d/%d', bb.exp[1], bb.exp[2])
        zts2:显示(
            self.x + ((255 - zts2:取宽度(ts)) / 2) + 8,
            self.y + 425,
            ts .. '(' .. 到整数(取百分比转换(bb.exp[1], 1, bb.exp[2]) * 100) .. '%)'
        )
    -- TODO 追加法术取消
    -- if bb.套装效果 ~= nil then
    --     self.追加法术:显示(self.x + 80 + 250, self.y + 150)
    --     if self.追加法术:是否选中(x, y) then
    --         tp.提示:自定义(self.x + 80 + 250, self.y + 150, bb.套装效果[2] .. bb.套装效果[1])
    --     end
    -- end
    end
    ...
    elseif self.资源组[19]:事件判断() then
        tp.窗口.召唤兽鉴定:打开(bb)
        tp.窗口.召唤兽鉴定.x = self.x + 370
        tp.窗口.召唤兽鉴定.y = self.y
    elseif self.资源组[19]:事件判断() then
        tp.窗口.召唤兽资质栏:打开(bb)
    ...
    elseif self.资源组[28]:事件判断() then --下滚动
        ...
        self.头像组 = {}
    elseif
        (self.资源组[29]:是否选中(x, y) or self.资源组[27]:是否选中(x, y) or self.资源组[35]:是否选中(x, y)) and 引擎.取鼠标滚轮() == 1 and
            self.加入 > 0
     then --鼠标上滚动
        self.资源组[35]:置起始点(self.资源组[35]:取百分比转换(self.加入 - 1, 4, bbsa))
        self.头像组 = {}
    elseif
        (self.资源组[29]:是否选中(x, y) or self.资源组[28]:是否选中(x, y) or self.资源组[35]:是否选中(x, y)) and 引擎.取鼠标滚轮() == -1 and
            self.加入 < bbsa - 4
     then --鼠标下滚动
        self.资源组[35]:置起始点(self.资源组[35]:取百分比转换(self.加入 + 1, 4, bbsa))
        self.头像组 = {}
    ...
    if self.拽拖对象 ~= 0 then
        ...
        if bbs[self.拽拖对象].join ~= nil and bbs[self.拽拖对象].join == false then
            zts:置颜色(-256)
        end
        zts:显示(x - 27, y - 19, bbs[self.拽拖对象].name)
        zts:显示(x - 27, y, bbs[self.拽拖对象].lv .. '级')
        if mouseb(0) then
            if self.鼠标 then
                if self.插入选区 == 0 then
                    self.拽拖计次 = 0
    ...
    if 开发调试 then
        ...
        for i = 1, 4 do
            if bbs[i + self.加入] ~= nil then
                ...
                if bbs[i + self.加入].被统御 ~= nil then
                    zts2:显示(jx + 91, jy + 21, bbs[i + self.加入].被统御 .. '号助战')
                end
            end
        end
        if bb ~= nil then
            ...
            if tp.队伍[1].宝宝列表[self.选中].种类 ~= nil then
                local 资源 = tp.资源
                if tp.队伍[1].宝宝列表[self.选中].种类 == '神兽' then
                    self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb118)
                    self.资源组[41]:更新(x, y)
                    self.资源组[41]:显示(self.x + 295, self.y + 145)
                elseif tp.队伍[1].宝宝列表[self.选中].种类 == '宝宝' then
                    self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb116)
                    self.资源组[41]:更新(x, y)
                    self.资源组[41]:显示(self.x + 295, self.y + 145)
                elseif tp.队伍[1].宝宝列表[self.选中].种类 == '仙灵' then
                    self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb117)
                    self.资源组[41]:更新(x, y)
                    self.资源组[41]:显示(self.x + 295, self.y + 145)
                elseif tp.队伍[1].宝宝列表[self.选中].种类 == '妖兽' then
                    self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb115)
                    self.资源组[41]:更新(x, y)
                    self.资源组[41]:显示(self.x + 295, self.y + 145)
                elseif tp.队伍[1].宝宝列表[self.选中].种类 == '至尊' then
                    self.资源组[41] = 资源:载入('wpal/121.wpal', '网易WDF动画', 0x00bbb114)
                    self.资源组[41]:更新(x, y)
                    self.资源组[41]:显示(self.x + 295, self.y + 145)
                else
                    --########################################################?自己修改?##########################################
                    ztstt3:显示(self.x + 204, self.y + 40, tp.队伍[1].宝宝列表[self.选中].种类)
                end
            end
            self.资源组[34]:显示(self.x + 320, self.y + 35)
            zts:置颜色(-16777216)
            ...
            -- ls
            zts:显示(self.x + 204, self.y + 247, bb.体质 + bb.装备属性.体质)
            if ls.体质 > 0 then
                zts:置颜色(-65536)
                zts:显示(self.x + 224 + (#tostring(bb.体质) * 7) + 1, self.y + 247, '+' .. ls.体质)
                zts:置颜色(-16777216)
            end
            zts:显示(self.x + 204, self.y + 271, bb.魔力 + bb.装备属性.魔力)
            if ls.魔力 > 0 then
                zts:置颜色(-65536)
                zts:显示(self.x + 224 + (#tostring(bb.魔力) * 7) + 1, self.y + 271, '+' .. ls.魔力)
                zts:置颜色(-16777216)
            end
            zts:显示(self.x + 204, self.y + 296, bb.力量 + bb.装备属性.力量)
            if ls.力量 > 0 then
                zts:置颜色(-65536)
                zts:显示(self.x + 224 + (#tostring(bb.力量) * 7) + 1, self.y + 295, '+' .. ls.力量)
                zts:置颜色(-16777216)
            end
            zts:显示(self.x + 204, self.y + 320, bb.耐力 + bb.装备属性.耐力)
            if ls.耐力 > 0 then
                zts:置颜色(-65536)
                zts:显示(self.x + 224 + (#tostring(bb.耐力) * 7) + 1, self.y + 319, '+' .. ls.耐力)
                zts:置颜色(-16777216)
            end
            zts:显示(self.x + 204, self.y + 344, bb.敏捷 + bb.装备属性.敏捷)
            if ls.敏捷 > 0 then
                zts:置颜色(-65536)
            ...
            zts2:显示(
                self.x + ((255 - zts2:取宽度(ts)) / 2) + 8,
                self.y + 425,
                ts .. '(' .. 到整数(取百分比转换(bb.当前经验, 1, bb.最大经验) * 100) .. '%)'
            )
