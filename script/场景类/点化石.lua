...
function 场景类_点化石道具栏:更新(dt,x,y)

 self.鼠标=self:检查点(x,y)
end
function 场景类_点化石道具栏:取可用道具(名称,物品)


end
function 场景类_点化石道具栏:显示(dt,x,y)
	if not self.可视 then
		return false
	end
	self.焦点 = false
	...
	zts1:显示(self.x+125,self.y+3,"点化宠物套装")

	--zts1:显示(self.x+125,self.y+3,"点化石")
	local n = self.开始-1
	local c =  false
	for h=1,4 do
		for l=1,5 do
			...
			if self.物品[n].物品~=nil and self.物品[n].焦点 then
				...
				if  self.物品[n].事件 and self.物品[n].物品.总类 == 2 and self.鼠标 then
			        发送数据(4520,{序列=self.物品[n].物品,序列1=self.数据.编号,序列2=n})
                    self.可视 = false
