...
-- @Last Modified time: 2020-10-01 13:09:43
...
function 给予类:打开(数据,名称,等级,类型)
	if self.可视 then
		...
		self.资源组 = {
			...
			[2] = 自适应.创建(0,1,270,450,3,9),
			[3] = 按钮.创建(自适应.创建(12,4,43,22,1,3),0,0,4,true,true,"取消"),
			[4] = 自适应.创建(1,1,270,18,1,3,nil,18),
			[5] = 自适应.创建(3,1,42,19,1,3),
			[6] = 自适应.创建(3,1,80,19,1,3),
		}
		self.队伍格子 = {}
	    ...
	    self.输入框:置文本("0")
	    self.文字显示 = require("gge文字类").创建(nil,16,false,false,true)
	    self.文字显示:置字间距(2)
	end
end
...
function 给予类:显示(dt,x,y)
	...
	tp.窗口标题背景_:显示(self.x+102,self.y+3)

	self.文字显示:显示(self.x+120,self.y+3,"给  予")
	tp.物品界面背景_:显示(self.x+5,self.y+32)
	tp.横排花纹背景_:置区域(3,0,260,18)
 	tp.横排花纹背景_:显示(self.x+5,self.y+247)
 	for i=1,3 do
 		tp.物品格子背景_:显示(self.x-50+i*81,self.y+277)
 		self.资源组[5]:显示(self.x-45+i*81,self.y+337)
 	end
 	self.文字显示:显示(self.x+5,self.y+362,"现金:")
	self.资源组[6]:显示(self.x+50,self.y+362)
	self.文字显示:显示(self.x+140,self.y+362,"银两:")
	self.资源组[6]:显示(self.x+185,self.y+362)
	...
	字体:显示(self.x + 50,self.y + 367,tp.金钱)
	字体:置颜色(黄色)
	字体:显示(self.x + 140,self.y + 394,"等级".."："..self.对象等级)
