...
-- @Last Modified time: 2020-12-26 12:57:28
--============================,==========================================--
local 场景类_成就提示栏1 = class()

local qmx = 引擎.取模型
local zts,zts1,tp
local insert = table.insert

function 场景类_成就提示栏1:初始化(根)
	self.ID = 14
	self.x = 300
	self.y = 500
	self.xx = 0
...
function 场景类_成就提示栏1:显示(dt,x,y)
	local cj = tp.成就提示1
	self.焦点 = false
	self.资源组[2]:更新()
	self.资源组[5]:更新()
	self.资源组[6]:更新()
	self.资源组[1]:显示(self.x,self.y-100)
	self.资源组[5]:显示(self.x,self.y-125)
	self.资源组[6]:显示(self.x,self.y-125)
	zts1:置字间距(0)
    ...
    zts:置颜色(红色)
    zts:显示(self.x + 10,self.y-95,"恭喜你获得了称谓："..cj[1])
    self.资源组[2]:显示(self.x+200,self.y-110)
