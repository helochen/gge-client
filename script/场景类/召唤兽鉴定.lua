...
--======================================================================--
local 场景类_召唤兽鉴定 = class()
local zts,zts1,bb,tp,资源,按钮,自适应
local insert = table.insert
local bds = {"攻击资质","防御资质","体力资质","法力资质","速度资质","躲闪资质","成长"}
local 取宝宝 = 引擎.取宝宝
local ani = 引擎.取战斗模型
...
function 场景类_召唤兽鉴定:初始化(根)
	...
	按钮 = 根._按钮
	自适应 = tp._自适应
	self.背景=资源:载入('wpal/121.wpal',"网易WDF动画",0x00000113)
...
function 场景类_召唤兽鉴定:刷新数据(b)
	bb=b
end

function 场景类_召唤兽鉴定:打开(b)
	if self.可视 then
...
function 场景类_召唤兽鉴定:显示(dt,x,y)
	...
	zts2:置颜色(绿色)
	zts2:显示(self.x+86,self.y+172,bb.性别)
	zts2:显示(self.x+86,self.y+191,bb.繁殖)
	if bb.元宵数量==nil then
	    bb.元宵数量=500
	end
	if bb.炼兽数量==nil then
	...
	zts2:置颜色(黄色)
	zts2:显示(self.x+81,self.y+209,bb.元宵数量)
	zts2:显示(self.x+149,self.y+209,bb.炼兽数量)
	zts2:显示(self.x+16,self.y+223,bb.千金数量)
	zts2:置颜色(白色)
	local 攻击上限=0
	local 防御上限=0
	local 体力上限=0
	...
	zts1:显示(self.x+101,self.y+250,""..bb.攻击资质)
	zts1:显示(self.x+101,self.y+279,""..bb.防御资质)
	zts1:显示(self.x+101,self.y+308,""..bb.体力资质)
	zts1:显示(self.x+101,self.y+337,""..bb.法力资质)
	zts1:显示(self.x+101,self.y+368,""..bb.速度资质)
	zts1:显示(self.x+101,self.y+398,""..bb.躲闪资质)
	zts1:显示(self.x+101,self.y+428,""..bb.成长)
	for i=1,6 do
		self.喂养[i]:更新(x,y)
		self.喂养[i]:显示(self.x+212 ,self.y+247+i+(i-1)*29 )
		if self.喂养[i]:事件判断() then
		    if bb.元宵数量>0 then
		        发送数据(3760,{元宵类型=i,序列=bb.认证码})
		    else
		    	tp.提示:写入("喂食元宵已达到上限,无法继续喂食")
		    end
		end
	end
	if self.使用珍经:事件判断() then
	    if bb.炼兽数量>0 then
		    发送数据(3761,{序列=bb.认证码})
		else
			tp.提示:写入("喂食炼兽珍经已达到上限,无法继续喂食")
		end
	end
end
function 场景类_召唤兽鉴定:置形象()
	self.宠物形象 = nil
	self.宠物饰品1 = nil
	self.宠物饰品2 = nil
	if bb ~= nil then
...
function 场景类_召唤兽鉴定:检查点(x,y)
	if self.背景:是否选中(x,y)  then
		return true
	end
end

function 场景类_召唤兽鉴定:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
