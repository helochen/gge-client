...
function 场景类_坐骑染色:加载数据()
	...
	self.资源组 = {
		...
		[3] = 资源:载入(tp.头像背景,"网易WDF动画",0x5B52CB27),
...
function 场景类_坐骑染色:显示(dt,x,y)
	...
	if self.鼠标 then
		...
		elseif self.资源组[5]:事件判断() then
			发送数据(3749,{序列=bb.认证码,序列1=bb.染色方案,序列2=bb.染色组[1],序列3=bb.染色组[2],序列4=bb.染色组[3]})
...
function 场景类_坐骑染色:检查点(x,y)
	if self.资源组 ~= nil and self.资源组[1]:是否选中(x,y)  then
		return true
	end
end

function 场景类_坐骑染色:初始移动(x,y)
	tp.运行时间 = tp.运行时间 + 1
