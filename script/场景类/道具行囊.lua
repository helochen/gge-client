   ...
   elseif self.窗口 == "锦衣" then
	...
	-- self:人物显示(dt,x,y)
	 ...
	 elseif self.资源组2[8]:事件判断() then
      ...
      local 佩戴坐标={{x=self.x+195,y=self.y+25},{x=self.x+255,y=self.y+25},{x=self.x+195,y=self.y+80},{x=self.x+255,y=self.y+80}}
	...
	elseif self.窗口 == "主人公" then
		...
		self.资源组[41]:显示(self.x + 170,floor(self.y + 4 * 25.5),nil,true,nil,tp.窗口.灵饰.可视,2)--灵饰
		self.资源组[60]:显示(self.x + 170,floor(self.y + 6 * 25.5),nil,true,nil,tp.窗口.显示开关.可视,2)--显示开关
		...
		self.资源组[40]:显示(self.x + 170,floor(self.y + 3 * 25.5),nil,true,nil,self.人物装备窗口==3,2)--装备3
		for i=1,6 do
			local 补差高度=3
			if i==1 or i == 2 then
			    补差高度=1
	...
	elseif self.窗口 == "召唤兽" or self.窗口 == "子女" then
		...
		if self.窗口 == "召唤兽" and dxs[self.选中召唤兽] ~= nil then
			self. 字体:显示(self.x + 209,self.y + 152 ,format("%s/%s",tp.队伍[1].宝宝列表[self.选中召唤兽].气血,tp.队伍[1].宝宝列表[self.选中召唤兽].最大气血))
			self. 字体:显示(self.x + 209,self.y + 172 ,format("%s/%s",tp.队伍[1].宝宝列表[self.选中召唤兽].魔法,tp.队伍[1].宝宝列表[self.选中召唤兽].最大魔法))
		end
	...
	elseif self.窗口 == "坐骑" then
		...
		if self.选中召唤兽 ~= 0 and tp.坐骑~=nil and tp.坐骑列表 ~= nil and tp.坐骑.认证码~=nil and tp.坐骑列表[self.选中召唤兽]~=nil and  tp.坐骑.认证码==tp.坐骑列表[self.选中召唤兽].认证码   then
			self.资源组[42]:置文字("下骑")
		else
			self.资源组[42]:置文字("骑乘")
		end
	...
	for h=1,4 do
		for l=1,5 do
			...
			if  tp.场景.地图.抓取物品 == nil and self.物品[o]~=nil and self.物品[o].焦点 and self.物品[o].物品 ~= nil and not tp.消息栏焦点 and self.物品[o].物品.分类~=nil and self.物品[o].物品.分类 <=12 and self.物品[o].物品.总类~=2 and self.物品[o].物品.总类~="坐骑饰品" then
			...
			if tp.场景.地图.抓取物品 == nil and self.物品[o].焦点 and self.物品[o].物品 ~= nil  then
				if mousea(1) then
					if self.物品[o].物品.总类 == 2  or self.物品[o].物品.总类==102 then
						if self.物品[o].物品.鉴定 or (self.物品[o].物品.分类 >= 10 and self.物品[o].物品.分类 <=14) or self.物品[o].物品.总类==102 then
							...
							elseif self.物品[o].物品 ~= nil and self.物品[o].物品.总类 == 102 and self.物品[o].物品.分类 >= 15 and self.物品[o].物品.分类 ~= 18 then
