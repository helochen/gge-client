...
-- @Last Modified time: 2020-10-11 22:01:34
--======================================================================--
...
function 场景类_鉴定道具栏:显示(dt,x,y)
	...
	zts1:显示(self.x+125,self.y+3,"装备分解")
	local n = self.开始-1
	local c =  false
	for h=1,4 do
		for l=1,5 do
			...
			if self.物品[n].物品~=nil and self.物品[n].焦点 then
				...
				if self.物品[n].事件 and self.物品[n].物品.总类 == 2 and self.鼠标 then
                    -- if self.物品[n].物品.鉴定 == false then
                    	if self.数据.编号 ~=nil and self.物品[self.数据.编号].物品.名称 == "分解符" then
                    	  --  if self.物品[n].物品.分类 == 3 or self.物品[n].物品.分类 == 4 then

                    	    	if self.物品[n].物品.级别限制==nil or self.物品[n].物品.级别限制<110 then
                    	    	    tp.提示:写入("#Y/低级装备无法分解")
                    	    	else
                    	    		发送数据(4510.1,{序列=self.物品[n].物品,序列1=self.数据.编号,序列2=n})
                    	    		self.可视 = false
                    	    	end
                    	    --else
