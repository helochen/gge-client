...
-- @Last Modified time: 2021-03-12 21:30:29
--======================================================================--
...
local function ew(mp)
   ...
   elseif mp == "神木林" then
	...
	elseif mp == "狮驼岭" then
		return "学习“背水”以后，每个额外的乾元丹会：增加30点速度。学习“肝胆”以后，每个额外的乾元丹会：增加80点防御。"
	elseif mp == "天宫" then
	...
	elseif mp == "女魃墓" then
		return "学习“觉醒”以后，每个额外的乾元丹会：增加30点速度。学习“无相诛戮”以后，每个额外的乾元丹会：增加40点法术伤害。"
	elseif mp == "天机城" then
		return "学习“铜山铁壁”以后，每个额外的乾元丹会：增加80点防御。学习“所向披靡”以后，每个额外的乾元丹会：增加40点伤害。"
	elseif mp == "花果山" then
		return "学习“战神”以后，每个额外的乾元丹会：增加40点伤害。学习“齐天神通”以后，每个额外的乾元丹会：增加80点防御。"
	end
end
...
function 场景类_奇经八脉:显示(dt,x,y)
	self.焦点 = false
	self.资源组[2]:更新(x,y)
	self.资源组[3]:更新(x,y,self.选中 ~= nil and self.数据.奇经八脉.剩余乾元丹 ~= 0)
	...
	elseif self.资源组[3]:事件判断() then--确认
		...
		self.数据.奇经八脉.剩余乾元丹 = self.数据.奇经八脉.剩余乾元丹 - 1
		if self.选中~=nil then
		    发送数据(32,{序列=self.选中})
			self.选中 = nil
		end
	...
	zts:置颜色(-256)
	zts:显示(self.x+335,self.y+391.8,format("乾元丹:%d",self.数据.奇经八脉.附加乾元丹))
	zts:显示(self.x+490,self.y+391.8,format(":%d",self.数据.奇经八脉.剩余乾元丹))
	zts:显示(self.x+583,self.y+391.8,format(":%d",self.数据.当前经验/99999999))
end
