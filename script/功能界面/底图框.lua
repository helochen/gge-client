...
function 系统类_底图框:加载()
    local 按钮 = tp._按钮
    local 资源 = tp.资源
    self.UI_底图 = 资源:载入(tp.底图框, '网易WDF动画', 0x3D1FA249)
    ...
    self.梦幻精灵 = 按钮.创建(资源:载入('other.wdf', '网易WDF动画', 0x61D49AEC), 0, 0, 4, true)
    self.商城底图 = 资源:载入('wzife.wd1', '网易WDF动画', 0x070155B1)
end
function 系统类_底图框:显示(dt, x, y)
    ...
    if not tp.战斗中 then
        ...
        elseif self.UI_动作:事件判断() then
            --tp.提示:写入("#Y/该功能暂时没开放")
        elseif self.UI_动作:事件判断() or (keyaz(KEY.ALT) and keyax(KEY.F)) and not tp.消息栏焦点 then
        ...
        elseif self.梦幻精灵:事件判断() or (keyaz(KEY.ALT) and keyax(KEY.D)) and not tp.消息栏焦点 then
        ...
        elseif self.UI_成就:事件判断() or (keyaz(KEY.ALT) and keyax(KEY.P)) and not tp.消息栏焦点 then
            if tp.窗口.成就系统栏.可视 == false then
                发送数据(22.1)
            else
                tp.窗口.成就系统栏:打开()
            end
        ...
        elseif self.UI_交易:事件判断() or (keyaz(KEY.ALT) and keyax(KEY.X)) and not tp.消息栏焦点 then
            tp.鼠标.置鼠标('交易')
        elseif
            (self.好友闪烁:是否选中(x, y) and 引擎.鼠标弹起(0)) or self.UI_好友:事件判断() or
                (keyaz(KEY.ALT) and keyax(KEY.F)) and not tp.消息栏焦点
         then
        ...
        elseif (keyaz(KEY.ALT) and keyax(KEY.V)) and 调试模式 then
            发送数据(99998)
        elseif
            (self.申请闪烁:是否选中(x, y) and 引擎.鼠标弹起(0)) or self.UI_队伍:事件判断() or
                (keyaz(KEY.ALT) and keyax(KEY.T)) and not tp.消息栏焦点
         then
        ...
        elseif (keyaz(KEY.CTRL) and keyax(KEY.Z)) then
            if 全局游戏宽度 <= 850 then
                引擎.置宽高(1200, math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720)
                全局游戏宽度 = 1200
                全局游戏高度 = math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720
            elseif 全局游戏宽度 <= 1200 and 全局游戏宽度 >= 850 then
                引擎.置宽高(1280, math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720)
                全局游戏宽度 = 1280
                全局游戏高度 = math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720
            elseif 全局游戏宽度 >= 1280 then
                引擎.置宽高(850, math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720)
                全局游戏宽度 = 850
                全局游戏高度 = math.ceil(读配置('./配置.ini', 'mhxy', '高度')) or 720
            end
            withs = 全局游戏宽度
        ...
        elseif (keyaz(KEY.CTRL) and keyax(KEY.X)) then
            if 全局游戏高度 <= 620 then
                引擎.置宽高(math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850, 720)
                全局游戏宽度 = math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850
                全局游戏高度 = 720
            elseif 全局游戏高度 == 720 then
                引擎.置宽高(math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850, 820)
                全局游戏宽度 = math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850
                全局游戏高度 = 820
            elseif 全局游戏高度 >= 768 then
                引擎.置宽高(math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850, 620)
                全局游戏宽度 = math.ceil(读配置('./配置.ini', 'mhxy', '宽度')) or 850
                全局游戏高度 = 620
            end
            withs = 全局游戏宽度
    ...
    if not tp.战斗中 then
        self.梦幻精灵:显示(全局游戏宽度 - 58, 全局游戏高度 - 132)
        self.商城底图:显示(全局游戏宽度 - 65, 全局游戏高度 - 133)
    end
