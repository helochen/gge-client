...
function 场景类_坐骑属性栏:打开()
	if self.可视 then
		...
		self.资源组 = {
			...
			[30] = 资源:载入(tp.造型背景,"网易WDF动画",0xA19838E8),--自适应.创建(2,1,158,173,3,9),--宠物显示框 158,153
...
function 场景类_坐骑属性栏:置形象()
	if tp.坐骑列表[self.选中]  ~= nil and tp.坐骑列表[self.选中].模型 ~= nil then
		local n = {}
		self.资源组[25] = nil
		self.资源组[40] = nil
		if 引擎.新增坐骑(tp.队伍[1].模型,tp.坐骑列表[self.选中].模型,"奔跑") ~= "" then
			n = {}
			n.人物资源 = "xzsc.wdf" or "wpal/121.wpal"
			n.人物站立 = 引擎.新增坐骑(tp.队伍[1].模型,tp.坐骑列表[self.选中].模型,"站立")
			n.坐骑资源 = "xzsc.wdf" or "wpal/121.wpal"
			n.坐骑站立 = 引擎.新增坐骑(tp.队伍[1].模型,tp.坐骑列表[self.选中].模型,"站立")
		elseif 引擎.取站立坐骑(tp.坐骑列表[self.选中].模型) ~= nil then
			n.坐骑资源 = "xzsc.wdf" or "wpal/121.wpal"
			n.坐骑站立 = 引擎.取站立坐骑(tp.坐骑列表[self.选中].模型)[2]
		else
			n = zqj(tp.队伍[1].模型,tp.坐骑列表[self.选中].模型,tp.坐骑列表[self.选中].饰品 or "空",tp.坐骑列表[self.选中].饰品2 or "空")--ani(tp.坐骑列表[self.选中].模型)
		end
		if  tp.队伍[1].坐骑 ~= nil and tp.坐骑列表[self.选中] ~= nil then
			self.资源组[36] = tp.资源:载入(n.人物资源,"网易WDF动画",n.人物站立)--人物
			if tp.坐骑列表[self.选中].模型 ~= "独角兽" and tp.坐骑列表[self.选中].模型 ~= "独角兽1" and tp.坐骑列表[self.选中].模型 ~= "飞天龙马"  and tp.坐骑列表[self.选中].模型 ~= "霜雪龙宝" and tp.坐骑列表[self.选中].模型 ~= "冰晶雪魄" and tp.坐骑列表[self.选中].模型 ~= "冰晶魅灵" and tp.坐骑列表[self.选中].模型 ~= "青梅竹马"  and tp.坐骑列表[self.选中].模型 ~= "深海狂鲨"  and tp.坐骑列表[self.选中].模型 ~= "独角马" and tp.坐骑列表[self.选中].模型 ~= "海豚恋人" and tp.坐骑列表[self.选中].模型 ~="冰晶魅灵" and tp.坐骑列表[self.选中].模型 ~="炫影天马" and tp.坐骑列表[self.选中].模型 ~="玲珑雪狐" and tp.坐骑列表[self.选中].模型 ~="玲珑雪狐女" and tp.坐骑列表[self.选中].模型 ~="烈焰雄狮" and tp.坐骑列表[self.选中].模型 ~="九尾神狐1" then
...
function 场景类_坐骑属性栏:刷新统御()
	local bbs = tp.坐骑列表
	if bbs[self.选中] ~= nil and bbs[self.选中].统御召唤兽~=nil  then
...
function 场景类_坐骑属性栏:显示(dt,x,y)
	...
	self.资源组[3]:显示(self.x+10,self.y+183)--观看
	...
	self.资源组[19]:显示(self.x+70,self.y+183,self.选中 ~= 0)--观看
	self.资源组[20]:显示(self.x+130,self.y+183,true)--祥瑞
	...
	if bb ~= nil then
		...
		if self.选中~=0 and tp.坐骑~=nil and tp.坐骑列表 ~= nil and tp.坐骑.认证码==tp.坐骑列表[self.选中].认证码 then
			self.资源组[3]:置文字("下骑 ")
		else
			self.资源组[3]:置文字("骑乘 ")
		end
	...
	elseif self.资源组[17]:事件判断() then --坐骑驯养
		if self.选中 == 0 then
			...
			tp.窗口.对话栏:文本("天兵","坐骑驯养","请选择您要进行的操作,单次驯养消耗1000W经验、100W银子",{"驯养一次","驯养十次","驯养五十次","算了算了"})
		end
