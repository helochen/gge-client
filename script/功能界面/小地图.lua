...
-- @Last Modified time: 2021-03-05 07:42:29
--======================================================================--
local 场景类_小地图 = class()
local tp
local xys = 生成XY
...
function 场景类_小地图:初始化(根)
    ...
    文字 = require('gge文字类')(wdf配置 .. '/font/hkyt.ttf', 16, 0, true)
    --(文件,大小,粗体,可描边,抗锯齿)
    文字:置字间距(1)
    文字:置描边颜色(0xFFFFFFFF)
    --白色 置描边颜色  置阴影颜色
    文字:置颜色(ARGB(255, 45, 74, 220))
    --0,40,247
    小地图等级 = 根.字体表.描边字体
...
function 场景类_小地图:显示(dt, x, y)
    ...
    if self.选择框显示[6] then
        self.迷你传送阵:更新(dt)
        for i = 1, #csdz do
            self.迷你传送阵:显示(floor(csdz[i].坐标.x * self.偏移X - 11 + self.x), floor(csdz[i].坐标.y * self.偏移Y + 11 + self.y))
        end
    end
    for n = 1, #jyjr do
        if jyjr[n].执行事件 ~= nil then
            jyjr[n].地图不显示 = nil
            jyjr[n].地图坐标 = {
                x = (floor(jyjr[n].坐标.x * self.偏移X + self.x - (pt:取宽度(jyjr[n].名称) / 2) + 6)),
                y = floor(jyjr[n].坐标.y * self.偏移Y + 4 + self.y)
            }
            for m = 1, #rwjr do
            ...
            if
                (self.普通显示 or self.选择框显示[1] or self.选择框显示[2] or self.选择框显示[3] or self.选择框显示[4] or self.选择框显示[5] or
                    self.选择框显示[6]) and
                    jyjr[n].地图不显示 == nil and
                    tp.场景.假人.执行事件 == nil
             then
    ...
    self.资源组[2]:显示(v + 16, self.y - 16)
    --关闭按钮
    if self.宽度 >= 500 then
        --世界按钮
        self.普通:显示(self.x - 5, v2 - 3, true)
        --全部选择框
        for i = 1, #self.选择框组 do
        ...
        self.普通:显示(v - 32, self.y + 13, true)
        --全部选择框
        for i = 1, #self.选择框组 do
    ...
    if mouseb(0) and tp.场景.人物 ~= nil and (tp.场景.人物.队长开关 == nil and #tp.队伍数据 >= 1) then
        ...
        local 角色坐标x = floor(tp.角色坐标.x * self.偏移X + self.x)
        -- + 13 + self.x)
        local 角色坐标y = floor(tp.角色坐标.y * self.偏移Y + self.y)
        --8 + self.y)
        local 鼠标坐标 =
            xys(
            floor((x - self.x) / (self.宽度 / (tp.场景.地图.宽度 / 20))),
            floor((y - self.y) / (self.高度 / (tp.场景.地图.高度 / 20)))
        )
        self.间隔计次 = 0
        if tp.场景 ~= nil and tp.场景.人物 ~= nil and #tp.场景.人物 ~= nil then
            for n = 1, #tp.场景.人物.目标格子 do
                ...
                if self.间隔计次 >= 15 and n ~= #tp.场景.人物.目标格子 then
                    self.临时坐标x = floor(tp.场景.人物.目标格子[n].x * 20 * self.偏移X + self.x)
                    -- + 13 + self.x)
                    self.临时坐标y = floor(tp.场景.人物.目标格子[n].y * 20 * self.偏移Y + self.y)
                    --8 + self.y)
            ...
            if
                self.小地图:是否选中(x, y) and self.鼠标 and not tp.消息栏焦点 and
                    (self.目标格子 == nil or (self.目标格子.x ~= x - self.x and self.目标格子.y ~= y - self.y))
             then
