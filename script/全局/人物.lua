...
function 场景类_人物:显示(dt,x,y,pys)
    ...
    引擎.置标题(全局游戏标题.." - "..tp.队伍[1].名称.."  [ID:"..tp.队伍[1].数字id.."] "..os.date(" %Y年%m月%d日 %H:%M:%S 星期")..gl天) --.."  [ID:"..tp.队伍[1].数字id.."] "  --修改id显示
	self:更新(dt,x,y,pys)
	人物点击=false
	if self.隐藏 then
		return
	...
	if self.人物["武器_"..self.行为] ~= nil then --武器偏移调整
	  ...
	  elseif self.人物模型 == "逍遥生" and self.方向 ~=0 and self.方向 ~=5 and self.方向 ~=7 and tp:取武器子类(tp.队伍[1].装备[3].子类)~="剑" then
...
function 场景类_人物:置模型2(开关)
	...
	if (tp.队伍[1].变身数据 == nil or 变身显示 == false) and 开关 then
		if tp.队伍[1].锦衣佩戴[1] ~= nil and tp.队伍[1].锦衣佩戴[1].名称 ~= nil  then
			if tp.队伍[1].锦衣佩戴[1].名称 == "夜影" or tp.队伍[1].锦衣佩戴[1].名称 == "夏日清凉" or tp.队伍[1].锦衣佩戴[1].名称 == "华风汉雅" or tp.队伍[1].锦衣佩戴[1].名称 == "萌萌小厨" then
...
function 场景类_人物:坐骑改变(模型,坐骑,饰品)
	...
	-- else
	if (tp.队伍[1].锦衣佩戴 == nil or tp.队伍[1].锦衣佩戴[1] == nil) and  坐骑.模型 ~= "独角兽" and 坐骑.模型 ~= "独角兽1" and 坐骑.模型 ~= "飞天龙马"  and 坐骑.模型 ~= "霜雪龙宝" and 坐骑.模型 ~= "冰晶雪魄" and 坐骑.模型 ~= "冰晶魅灵" and 坐骑.模型 ~= "青梅竹马"  and 坐骑.模型 ~= "深海狂鲨"  and 坐骑.模型 ~= "独角马" and 坐骑.模型 ~= "海豚恋人" and 坐骑.模型 ~="冰晶魅灵" and 坐骑.模型 ~="炫影天马" and 坐骑.模型 ~="玲珑雪狐" and 坐骑.模型 ~="玲珑雪狐女" and 坐骑.模型 ~="烈焰雄狮" and 坐骑.模型 ~="九尾神狐1" then
	    self:置染色(tp.队伍[1].染色方案,tp.队伍[1].染色组[1],tp.队伍[1].染色组[2],tp.队伍[1].染色组[3])
	end
	if tp.队伍[1].锦衣佩戴[3] ~= nil and tp.队伍[1].锦衣佩戴[3].名称 ~= nil and (tp.队伍[1].变身数据 == nil or 变身显示 == false) then
		self:穿戴光环()
